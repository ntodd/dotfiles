...
globs: ["*.ex", "*.exs", "*.heex"]
condition: '\b(?:redirect|push_navigate|push_patch)\s*\([^)]*\bto:\s*(?:"/|\{\s*"/)|(?:href|navigate|patch)\s*=\s*(?:"/|\{\s*"/)|\bRoutes\.[a-z_]\w*_(?:path|url)\s*\(|\b(?!static_)[a-z_]\w*_(?:path|url)\s*\(\s*(?:@?conn|@?socket|[A-Z][A-Za-z0-9_.]*Endpoint)\b'
